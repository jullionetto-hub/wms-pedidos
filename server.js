...
app.post('/auth/login', (req, res) => {
  ...
  db.get(
    ...
    (err, user) => {
      ...
      if (perfil === 'separador') {
        db.get(`SELECT * FROM separadores WHERE usuario_id=? AND status='ativo'`,
          [user.id], (err2, sep) => {
            if (err2) return res.status(500).json({ erro: err2.message });
            res.json({ usuario: { id:user.id, nome:user.nome, login:user.login, perfil:user.perfil }, separador: sep||null });
          });
      } else {
        res.json({ usuario: { id:user.id, nome:user.nome, login:user.login, perfil:user.perfil }, separador: null });
      }
    }
  );
});
...
app.get('/usuarios', (req, res) => {
  ...
  let sql = 'SELECT id,nome,login,perfil,status,turno,data_cadastro FROM usuarios WHERE 1=1';
  const params = [];
...
app.post('/usuarios', (req, res) => {
  ...
  db.run(`INSERT INTO usuarios (nome,login,senha_hash,perfil,turno) VALUES (?,?,?,?,?)`,
    [nome, login, hash, perfil, turno||'Manhã'], function(err) {
      if (err) {
...
app.put('/usuarios/:id', (req, res) => {
  const { nome, login, senha, perfil, status, turno } = req.body;
  if (senha) {
    const hash = hashSenha(senha);
    db.run(`UPDATE usuarios SET nome=?,login=?,senha_hash=?,perfil=?,status=?,turno=? WHERE id=?`,
      [nome,login,hash,perfil,status,turno||'Manhã',req.params.id], err => {
        if (err) return res.status(500).json({ erro: err.message });
        res.json({ mensagem: 'Usuário atualizado!' });
      });
  } else {
    db.run(`UPDATE usuarios SET nome=?,login=?,perfil=?,status=?,turno=? WHERE id=?`,
      [nome,login,perfil,status,turno||'Manhã',req.params.id], err => {
        if (err) return res.status(500).json({ erro: err.message });
        res.json({ mensagem: 'Usuário atualizado!' });
      });
  }
...
app.get('/separadores/matricula/:matricula', (req, res) => {
  db.get('SELECT * FROM separadores WHERE matricula=? AND status="ativo"',
    [req.params.matricula], (err, row) => {
      if (err)  return res.status(500).json({ erro: err.message });
...
app.post('/separadores', (req, res) => {
  ...
  db.run('INSERT INTO separadores (nome,matricula,turno,usuario_id) VALUES (?,?,?,?)',
    [nome, matricula, turno||'Manhã', usuario_id||null], function(err) {
      if (err) return res.status(500).json({ erro: err.message });
...
app.put('/separadores/:id', (req, res) => {
  ...
  db.run('UPDATE separadores SET nome=?,matricula=?,turno=?,status=?,usuario_id=? WHERE id=?',
    [nome,matricula,turno,status,usuario_id||null,req.params.id], err => {
      if (err) return res.status(500).json({ erro: err.message });
      ...
      db.run('UPDATE itens_pedido SET status=?,obs=?,qtd_falta=?,hora_verificado=? WHERE id=?',
        [status,obsTexto,qtdFaltou,hora,req.params.id], err2 => {
          ...
          if (status==='falta'||status==='parcial') {
            ...
            db.get(`SELECT id FROM avisos_repositor WHERE item_id=? AND status='pendente'`,
              [item.id], (err3, jaExiste) => {
                if (jaExiste) {
...
app.put('/pedidos/:id/concluir', (req, res) => {
  db.all(`SELECT * FROM itens_pedido WHERE pedido_id=? AND status='pendente'`,
    [req.params.id], (err, pendentes) => {
      if (err) return res.status(500).json({ erro: err.message });
      if (pendentes.length>0)
        return res.status(400).json({ erro:`Ainda há ${pendentes.length} item(s) não verificado(s)!` });
      db.all(`SELECT * FROM avisos_repositor WHERE pedido_id=? AND status='pendente'`,
        [req.params.id], (err2, avisosPendentes) => {
          if (err2) return res.status(500).json({ erro: err2.message });
...
app.put('/repositor/avisos/:id/reposto', (req, res) => {
  const hora = new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
  db.run('UPDATE avisos_repositor SET status="reposto",hora_reposto=? WHERE id=?',
    [hora,req.params.id], err => {
      if (err) return res.status(500).json({ erro: err.message });
...
app.put('/repositor/avisos/:id/nao_encontrado', (req, res) => {
  const hora = new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
  db.run('UPDATE avisos_repositor SET status="nao_encontrado",hora_reposto=? WHERE id=?',
    [hora,req.params.id], err => {
      if (err) return res.status(500).json({ erro: err.message });
...
app.post('/importar', (req, res) => {
    ...
    db.run(
      ...
      function(err) {
        ...
        db.get('SELECT id FROM pedidos WHERE numero_pedido=?', [numero], (err2,pedido) => {
          if (err2||!pedido) { erros++;processados++;verificarFim();return; }
          if (this.changes>0) importados++; else ignorados++;
          const stmt = db.prepare(
            'INSERT OR IGNORE INTO itens_pedido (pedido_id,codigo,descricao,endereco,quantidade) VALUES (?,?,?,?,?)'
          );
          itens.forEach(item => stmt.run([pedido.id,
            String(item.codigo||'').trim(),String(item.descricao||'').trim(),
  ...
  function verificarFim(){
    if (processados===numeros.length)
      res.json({ mensagem:'Importação concluída!', importados, ignorados, total:numeros.length });
  }
});
...
app.listen(3000, '0.0.0.0', () => console.log('Servidor WMS rodando em http://localhost:3000'));